...
- **Admin page** `admin/index.html` (`/admin/`, `noindex`, `sitemap: false`) + `assets/js/admin.js`: a Chrome-only form that writes directly into the local checkout via the File System Access API (directory handle persisted in IndexedDB). Tabs: new project (writes `_projects/<slug>.md` + copies images to `img/<slug>_N.ext`, cover = `_0`), non-project CV item (text-inserts under the chosen section's `items:` in `_data/cv.yml`), bio (`_data/bio.yml`). It can also load an existing project (`parseProject`/`serializeProject` handle the front-matter subset the site uses: scalars, `tags: [..]`, `media:` list incl. `bandcamp: { album|track }`, `links:`; unknown keys are preserved verbatim, comments dropped) and overwrite it; body is edited as raw markdown in that mode. `publish.sh` = `_cv/build.sh` + commit + push; `preview.sh` = livereload serve + `_admin_helper.py` (a 127.0.0.1:4001 HTTP helper with `GET /status` = `git status --short` and `POST /publish` = run `publish.sh`; Origin-checked against the local preview, excluded from the build) so the admin page's "사이트에 올리기" button can publish. English fields get "translate from Korean" buttons backed by Chrome's built-in on-device Translator API (ko→en; needs a user gesture for the first model download; no keys). Keep the front-matter format the admin emits in sync with `_template.md` and the CV include.
