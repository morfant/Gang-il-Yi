...
- **Admin page** `admin/index.html` (`/admin/`, `noindex`, `sitemap: false`) + `assets/js/admin.js`: a Chrome-only form that writes directly into the local checkout via the File System Access API (directory handle persisted in IndexedDB). Tabs: new project (writes `_projects/<slug>.md` + copies images to `img/<slug>_N.ext`, cover = `_0`), non-project CV item (text-inserts under the chosen section's `items:` in `_data/cv.yml`), bio (`_data/bio.yml`). It never edits existing project files. `publish.sh` = `_cv/build.sh` + commit + push; `preview.sh` = livereload serve. Keep the front-matter format the admin emits in sync with `_template.md` and the CV include.
